...
from db_connection import conectar_bd, obtener_usuario_id  # Importa las funciones necesarias
...
def iniciar_sesion():
    ...
    if connection:
        cursor = connection.cursor()
        cursor.execute("SELECT usuario_id FROM usuarios WHERE usuario = %s AND clave = %s", (usuario, clave))
        usuario_id = cursor.fetchone()

        if usuario_id:
            messagebox.showinfo("Inicio de Sesión Exitoso", f"Bienvenido, Usuario ID: {usuario_id[0]}")
...
imagen_fondo = Image.open("C:/Users/ValeriaBravo/Documents/PROYECTOS PYTHON/Proyecto TEA Actual1.1/Proyecto TEA Actual1.1/Proyecto TEA Actual/Imagenes/fondo2.jpg")
